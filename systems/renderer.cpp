...
namespace muyuy::ecs::systems
{
    ...
    void Renderer::characters(entt::registry &reg, Rect &camera)
    {
        auto view = reg.view<components::Animation, components::Character, components::Position, components::Sprite>();
        for (auto e : view)
        {
    ...
    {
        auto view = reg.view<components::Animation, components::Walker, components::Position, components::Sprite>();
        for (auto e : view)
        {
            int id = view.get<components::Animation>(e).current_frame.first - 1;
            auto offset_x = view.get<components::Sprite>(e).width * ((id % view.get<components::Sprite>(e).cols) - 1);
